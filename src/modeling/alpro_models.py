...
class AlproForSequenceClassification(AlproBaseModel):
    def __init__(self, config, video_enc_cfg, input_format='RGB'):
        ...
        ##@@
        print('Setup MCQ in ALPRO model init 1')
        print('Setup MCQ in ALPRO model init 2')
        print('Setup MCQ in ALPRO model init 3')

        ##@@ manually parameter setting
        model_clip = torch.load('/content/drive/MyDrive/Colab projects/VideoQA research projects/OnlineVideoQA/OVQA-ALPRO/data/MCQ_PT/MCQ_CLIP.pth', map_location="cuda:0")
        state_dict = model_clip['state_dict']
        self.mcq_model = build_model(state_dict)

        for param in self.mcq_model.parameters():
            param.requires_grad = False

    # def forward(self, image, text, targets, alpha=0, train=True):
    def forward(self, batch):
        ##@@ get visual inputs in forward process
        visual_inputs = [] # (B, )

        for i, video_path in enumerate(batch["video_paths"]):

            ##@@ print(f'fowarding video_path{video_path}')
            frame_indices, visual_input = self.online_fib_matching_frames_decord(video_path=video_path,
                                                                                 text=batch["q_strs"][i],
                                                                                 ret_model=self.mcq_model,
                                                                                 c_level=0.400,
                                                                                 fib_level=4)
            # list, #(1, T,C,H,W)
            visual_input = visual_input.squeeze(0)
            ##@@ print(f'fowarding visual_input {visual_input.shape}')
